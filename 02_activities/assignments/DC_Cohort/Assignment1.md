...
```
Databases and data systems often present themselves as neutral, but in reality, they reflect the assumptions of the people that build them. If a company uses data from decades ago to decide what a 'good employee' looks like, that data would be based on older hiring patterns that may have excluded certain racial groups. In that case, the database is reinforcing inequality and using it as a model for future decision-making. This shows me that technology can often be intertwined with society. Another example is medical school admissions. The databases involved are built around decisions about what counts as 'merit', and that is shaped in large part by social inequality and unequal access to resources. Students from wealthier backgrounds have more time for unpaid volunteering and better support navigating the admissions process. If these data systems are built around criteria that historically favored more privileged groups, it can reproduce these inequalities. In fact, in 2024, the Association of Faculties of Medicine of Canada stated that the MCAT has been shown to discriminate based on ethnicity and socioeconomic status. It thus recommended that Canadian medical schools discontinue it as an admission requirement.
Privacy is another  value system embedded in databases. Most of the databases we interact with on a daily basis (apps on our phones like Google Maps) are designed to collect and store as much information about people as possible. Sometimes, these data can be shared across other systems and used for purposes beyond the original reason it was collected. This reflects a value system where institutional efficiency and profit are prioritized over people's control over their own information. These examples show that databases are not simply tools for storing information. They shape how people are evaluated, categorized, and treated in society.
```
